...
async function assembleContext(ticket, logger = null) {
  ...
  const [notionResult, zendeskResult, glossaryResult, historyResult, improvementsResult] = await Promise.allSettled([
    fetchNotionContext(searchQuery, logger),
    fetchZendeskContext(searchQuery, logger),
    getGlossary(detectedLanguage, logger),
    getTicketHistory(ticket, logger),
    fetchImprovementInsights(logger),
  ]);
