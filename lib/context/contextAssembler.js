...
/**
 * Extract English search keywords from a ticket for knowledge base lookups.
 * Uses the ticket subject + first customer message as input so the model has
 * enough context to produce meaningful English terms.
 * Only called for non-English tickets. Returns the raw query on failure.
 */
async function translateSearchQuery(ticket, rawQuery, logger = null) {
  try {
    const subject = ticket.subject || "";
    const firstCustomerMsg = (ticket.comments || [])
      .find(c => c.author?.role !== "agent")?.value || "";
    const context = [subject, firstCustomerMsg.slice(0, 300)].filter(Boolean).join(" ");

    const response = await openai.chat.completions.create({
      model: "gpt-4o-mini",
      messages: [{
        role: "user",
        content: `What is this customer support ticket about? Reply with 4-5 English words only that describe the topic for a fulfillment/logistics knowledge base search. No non-English words.\n\nTicket: ${context}`,
      }],
      max_tokens: 30,
      temperature: 0,
    });
    const translated = response.choices[0]?.message?.content?.trim() || rawQuery;
    logger?.log("context", "Search query translated to English", { original: rawQuery, translated });
    return translated;
  } catch (err) {
    logger?.log("error", "Search query translation failed, using original", { error: err.message });
    return rawQuery;
  }
}
...
async function assembleContext(ticket, logger = null) {
  ...
  // For non-English tickets, translate the search query to English so Notion/FAQ
  // searches find relevant content. Runs in parallel with entity/glossary/history.
  // English tickets skip the API call entirely.
  const englishQueryPromise = detectedLanguage !== "en"
    ? translateSearchQuery(ticket, searchQuery, logger)
  ...
  const [notionResult, zendeskResult, glossaryResult, historyResult, improvementsResult, hiveResult] = await Promise.allSettled([
    withTimeout(englishQueryPromise.then(q => fetchNotionContext(q, logger)), 10000),
    withTimeout(englishQueryPromise.then(q => fetchZendeskContext(q, logger)), 7000),
 ...
 * Sources are returned even if page content fetching times out.
 */
async function fetchNotionContext(searchQuery, logger = null) {
  ...
  // Fetch content separately with its own 5s limit.
  // If content is slow or fails, sources are still returned.
  try {
    const topPages = pages.slice(0, 2);
    const pagesWithContent = await Promise.race([
      getContextFromPages(topPages, logger),
    ...
    logger?.log("notion", "Content fetch timed out, returning sources only", { error: err.message });
    return { context: "", sources };
  }
}
