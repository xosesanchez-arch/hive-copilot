 ...
 * This is the most reliable method — the ticket is already linked to a merchant in Zendesk.
 */
async function resolveMerchant(ticket, logger = null) {
  const ticketId = ticket.id;

  if (!ticketId) {
    logger?.log("hive", "No ticket ID to resolve merchant");
    return null;
  }
...
function formatOrderForLLM(order) {
  const lines = [
    ...
    `- Reference: ${order.external_name || "N/A"}`,
  ...
  if (order.shipping_address) {
    ...
    lines.push(`- Recipient: ${[a.name, a.city, a.country].filter(Boolean).join(", ")}`);
  }
...
function formatRestockingForLLM(restock) {
  const lines = [
    ...
    `- Expected Arrival: ${restock.expected_arrival_date || "N/A"}`,
  ];

  if (restock.discrepancies?.length > 0) {
    lines.push(`- Discrepancies: ${restock.discrepancies.length} item(s) with discrepancies`);
  }
      ...
      callTool("get_order", { merchant_id: merchantId, order_id: orderId }, logger)
        .then((order) => {
          if (order) {
            ...
            results.entitySources.push({ type: "order", id: orderId, name: order.external_name || orderId });
          }
        })
