 ...
 */
async function generateNextSteps(ticket, notionContext = "", faqContext = "", entityContext = "", logger = null) {
  ...
  // Add entity context if available — next steps should reference real entity data
  if (entityContext) {
    messages.push({
      role: "system",
      content: `## Real-time Entity Data (from Hive system):\n\n${entityContext}\n\nUse the actual status, IDs, and details above when recommending next steps.`,
    });
  }
